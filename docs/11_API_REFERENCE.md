...
| MAT-1 | GET /matches | org | state=new\|saved, limit (**cursor not built**) | **200** ranked matches (tender summary + score + explanation + `eligibility`) — one per opportunity GROUP, never per source row (ADR-028). `state` cannot be set to `dismissed`: FR-7.3 means no listing may offer them | **403 `audience_restricted`** (local org, ADR-029) |
| MAT-2 | POST /matches/{id}/save | org | `X-CSRF-Token` | **200** the updated MatchOut (superset of the specified `{state:"saved"}`, so a client redraws the card without a refetch) | 404 (also for another org's match) · **409 expired not built** — matches have no expiry concept yet |
| MAT-3 | POST /matches/{id}/dismiss | org | `X-CSRF-Token` | **200** the updated MatchOut, `state:"dismissed"`; the row survives so ranking still knows the group was judged, and never resurfaces (FR-7.3) | 404 |
