...
| — | `COMPETITORS.md` | The Ethiopian tender-platform landscape (GetChereta, 2Merkato, AfroTender, EthiopianTender, e-GP, GlobalTenders); mirrored in `adera-web`/`adera-mobile` | Before any first-task competitive research; before roadmap calls on features another platform already ships |
| — | `QUALIFICATION_PREFILTER.md` | Open problem statement for the qualification prefilter (FR-5.1/5.2) — the actual next backend build ticket; the design is not yet decided | Before building the qualification step |
