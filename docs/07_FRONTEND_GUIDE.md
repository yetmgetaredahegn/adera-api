...
Public pages < 100 KB first-load JS (NFR/UX §7 master plan): Server Components by default, `next/dynamic` for QAChat/admin, `next/font` self-hosted Inter + Noto Sans Ethiopic (subset!), no hero images (design is typographic — 08), Tailwind purges itself. Enforce: `next build` bundle output eyeballed per PR + Lighthouse CI budget (perf ≥ 90 on /, /tenders/[slug]) in the pipeline — a regression fails the build, same as a test.
